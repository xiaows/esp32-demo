...
- 启动顺序、文件职责、开发部署、量产思路：`docs/deployment.md`
...
Web IDE 内置了命令耗时统计功能，从命令发出到收到最终响应（`SUCCESS` 或 `ERROR`）自动计时，结果显示在输出日志中。
...
2. 选择 BLE 或 USB 连接 ESP32

3. 执行以下操作，观察日志中 `[耗时: xxxms]` 的输出：

| 测试项 | 操作 | 日志示例 |
|--------|------|----------|
| 简单执行 | 编辑器输入 `print('hi')` → 点击执行 | `✓ 代码执行完成 [耗时: 156.3ms]` |
| 文件列表 | 点击「列出文件」 | `✓ 找到 3 个文件 [耗时: 42.7ms]` |
| 文件保存 | 点击「保存代码到ESP32」 | `✓ 文件保存成功 [耗时: 320.1ms]` |
| 文件删除 | 在文件列表中点击删除 | `✓ 文件已删除 [耗时: 38.5ms]` |
| 长代码执行 | 执行包含循环和 `should_stop()` 的代码 | 耗时覆盖完整执行周期 |

### 计时说明

- 计时起点：浏览器发出命令的瞬间（`performance.now()`）
- 计时终点：收到 `SUCCESS` 或 `ERROR` 响应
- 中间的 `INFO`、`OUTPUT` 响应不会中断计时
- 耗时包含：BLE/USB 传输 + ESP32 解析 + 代码执行 + 响应回传
- 点击日志区右上角「清除」按钮可清空日志
